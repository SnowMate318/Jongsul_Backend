...
class Question(BaseModel):
    ...
    choices: List[Choice] = Field(description='''
                                  객관식일 경우 4개의 선택지 번호와 선택지 내용을 제공.
                                  주관식일 경우 빈 리스트로 제공.
                                  ox문제일 경우 빈 리스트로 제공.
                                  ''')
    question_answer: str = Field(description='''
                                객관식일 경우 선택지 번호를 제공(1~4),
                                주관식일 경우 정답 내용을 제공(한 단어로 대답),
                                ox문제일 경우 O/X로 대답
                                 ''')
...
getQuestions(concept1, 5, 2, 3, 1, 6)
...
# class Question(BaseModel):
#     """
#         문제에 대한 정보를 담고 있는 클래스
#         만약 question_type이 1이라면, 크기가 4인 choices 리스트를 가져야 함
#         만약 question_type이 2라면, choices 리스트는 빈 리스트가 되어야 함
#         만약 question_type이 3이라면, choices 리스트는 빈 리스트가 되어야 함
#     """
#     question_num: int = Field(description="문제 번호(1부터 하나씩 증가)")    
#     question_title: str = Field(description="문제 제목")
#     question_type : int = Field(description = "문제 유형")
#     choices: List[Choice] = Field(description="선택지")
#     question_answer: str = Field(description="문제 정답")
#     question_explanation: str = Field(description="문제 해설")
        
# class Problems(BaseModel):
#     """question에 대해 추출된 데이터."""
#     questions: List[Question]
...
#             "system",
#             "너는 문제를 만들어주는 알고리즘이야"
#             "입력받은 개념과, 해당 개념과 관련된 네가 알고있는 지식을 기반으로 문제를 만들어줘."
#             "입력받은 문자열이 개념이나 정보와 관련되지 않을 경우 null을 반환해."
#             "문제를 만들 때, 난이도를 입력받아서 해당 난이도에 맞는 문제를 만들어줘. 난이도는 1붙여 10까지의 숫자로 표현되며, 숫자가 클수록 어려운 문제를 만들어줘."
#             "너는 오직 3가지 유형에 대한 문제만 만들 수 있어."
#             "type 1은 객관식 문제야."
#             "객관식 문제는 제시된 4개의 선택지 중 정답에 해당하는 선택지를 찾아내는 문제야."
#             "type 2는 단답형 문제야."
#             "단답형 문제는 주어진 문제에 대한 답을 짧게 쓰는 문제야."
#             "type 3은 ox 문제야"
#             "ox 문제는 개념에 대해 참인 문장 또는 거짓인 문장을 제시해주고 제시된 문장이 참인지 거짓인지를 판단하는 문제야."
#             "너는 문제유형 별 문제갯수만큼의 문제를 만들어줘야 해."
#             '''
#             예를들어, 
#             객관식 문제 갯수: 1, 단답형 문제 갯수: 2, ox문제 갯수: 3이라면,
#             객관식 문제를 1문제 만들고, 단답형 문제를 2문제 만들고, ox문제를 3문제 만들어.
#             예를들어,
#             객관식 문제 갯수: 3, 단답형 문제 갯수: 3, ox문제 갯수: 2이라면,
#             객관식 문제를 3문제 만들고, 단답형 문제를 3문제 만들고, ox문제를 2문제 만들어.
#             '''
#             "한글로 만들어줘."
#             ), 
#             ("user", "{input}")]
...
#     chain = prompt | model.with_structured_output(schema=Problems, method="function_calling", include_raw=False,) | extract_json
#     try:
#         with get_openai_callback() as cb:
#             res = chain.invoke({
#                 "input": f''' 
#                 개념: {script}, 
#                 난이도: {difficulty},
#                 객관식 문제 갯수: {multiple_choice}, 단답형 문제 갯수: {short_answer}, ox문제 갯수: {ox_prob}.
#                 ''',
#                 })
#                 #print(res)
#     except Exception as e:
...
# 상업 : 벽란도 - 아라비아 상인과 교류(코리아)
# 기타 용어 : 혜민국, 삼국사기(김부식, 전기), 삼국유사(후기)'''
# getQuestions(concept2, 5, 2, 3, 1, 6)